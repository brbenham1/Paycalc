...
export const getHoursAndMinutesBetween = (startTime: string, endTime: string): TimeDifference => {
...
export const calculateTotalHoursAndMinutes = (shifts: ShiftInformation[]): TimeDifference => {
	return shifts.reduce(
		(acc, shift) => {
			const { hours, minutes } = getHoursAndMinutesBetween(shift.startTime, shift.endTime);
			const totalMinutes = acc.minutes + minutes;
			return {
				hours: acc.hours + hours + Math.floor(totalMinutes / 60),
...
export const durationString = (shift: ShiftInformation) => {
	const { hours, minutes } = getHoursAndMinutesBetween(shift.startTime, shift.endTime);
	return getDurationString(hours, minutes);
};
